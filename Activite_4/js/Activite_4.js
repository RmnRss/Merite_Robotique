...
(function(ext) {
  ...
  ext.getBinaryIn = function(nextID, entree_binaire_select){
    var deviceId = 30;
    getPackage(nextID,deviceId,entrees_binaires[entree_binaire_select]);
  };

  ext.setBinaryOutHigh = function(sorties_binaires_select) {
    runPackage(30,sorties_binaires[sorties_binaires_select],1);
  };

  ext.setBinaryOutLow = function(pin,level) {
    runPackage(30,sorties_binaires[sorties_binaires_select],0);
  };
  ...
  function deviceOpened(dev) {
      ...
      device.set_receive_handler('Activite_4',function(data) {
          processData(data);
      });
  };
  ...
  ext._getStatus = function() {
      if(!device) return {status: 1, msg: 'Extension Activite_4 déconnectée'};
      return {status: 2, msg: 'Extension Activite_4 connectée'};
  };

  var descriptor = {};

	ScratchExtensions.register('Activite_4', descriptor, ext, {type: 'serial'});
})({});
